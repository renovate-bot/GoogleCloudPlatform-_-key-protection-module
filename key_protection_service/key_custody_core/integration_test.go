...
func TestIntegrationGetKEMKey(t *testing.T) {
	...
	if remainingLifespanSecs == 0 {
		t.Fatal("expected non-zero remainingLifespanSecs")
	}
	if remainingLifespanSecs > 3600 {
		t.Fatalf("expected remainingLifespanSecs <= 3600, got %d", remainingLifespanSecs)
	}
}
